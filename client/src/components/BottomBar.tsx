...
import { Users, Moon, School, Bot, User } from "lucide-react";
...
export const BottomBar: React.FC = () => {
  ...
  const { campus, companion } = useBadges();
  ...
  // Hide bottom bar on character chat pages, dark room, campus pages, hangout pages, and room interfaces
  ...
  if (location.pathname.startsWith('/arena/hangout/chat/')) return null; // Hide in hangout chat rooms
  if (location.pathname.startsWith('/arena/hangout/info/')) return null; // Hide in hangout info pages
  if (location.pathname.startsWith('/arena/hangout/admin/')) return null; // Hide in admin panel pages
  ...
  return (
    <>
      ...
      >
        <div className="bg-zinc-900/95 backdrop-blur-xl rounded-3xl border border-softgold-500/40 shadow-2xl max-w-2xl md:max-w-xl mx-auto overflow-hidden">
              ...
              return (
                <button
                  ...
                  className={`relative flex flex-col items-center justify-center min-w-0 flex-1 py-2 px-1.5 sm:py-2.5 sm:px-2 md:py-1.5 md:px-1.5 rounded-2xl md:rounded-xl transition-all duration-300 ${
                    active 
                      ? "bg-softgold-500 text-black scale-105" 
