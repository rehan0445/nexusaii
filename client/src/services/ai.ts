...
export const generateResponse = async (message: string, characterId: string) => {
  try {
    ...
    // Return a more helpful error message with error details for debugging
    if (errorMessage.includes('VENICE_API_KEY') || errorMessage.includes('API_KEY_MISSING')) {
    ...
    } else if (errorMessage.includes('QUEUE_FULL')) {
