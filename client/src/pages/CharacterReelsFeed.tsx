...
const CharacterReelsFeed: React.FC<CharacterReelsFeedProps> = ({ 
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 bg-[#141414] flex items-center justify-center z-50">
        <div className="text-white text-xl font-medium">Loading characters...</div>
      </div>
    );
  }
  ...
  if (charactersArray.length === 0) {
    return (
      <div className="fixed inset-0 bg-[#141414] flex items-center justify-center z-50">
        <div className="text-white text-xl font-medium">No characters available</div>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 bg-[#141414] z-50 overflow-hidden">
      {/* Desktop background overlay */}
      <div className="hidden md:block fixed inset-0 bg-gradient-to-br from-[#0A0A0A] via-[#141414] to-[#0A0A0A]" />
      ...
      <div className="relative w-full h-full md:flex md:items-center md:justify-center md:py-4 md:px-4">
        ...
        <div className="w-full h-full md:w-[420px] md:h-[calc(100vh-2rem)] md:max-h-[800px] md:rounded-xl md:overflow-hidden bg-[#0A0A0A] md:shadow-2xl relative md:border md:border-white/10">
          ...
          <button
            ...
            className="absolute top-4 left-4 z-50 p-2 rounded-full bg-[#0A0A0A]/80 backdrop-blur-md text-white hover:bg-[#1A1A1A] border border-white/10 transition-all md:hidden"
            aria-label="Back"
          >
          ...
          {!onClose && (
            <button
              ...
              className="absolute top-4 left-4 z-50 p-2 rounded-full bg-[#0A0A0A]/80 backdrop-blur-md text-white hover:bg-[#1A1A1A] border border-white/10 transition-all hidden md:flex items-center gap-2"
              aria-label="Back"
            >
              <ArrowLeft className="w-5 h-5" />
          ...
          {onClose && (
            <button
              onClick={onClose}
              className="absolute top-4 right-4 z-50 p-2 rounded-full bg-[#0A0A0A]/80 backdrop-blur-md text-white hover:bg-[#1A1A1A] border border-white/10 transition-all hidden md:block"
              aria-label="Close"
            >
              <X className="w-6 h-6" />
            </button>
          ...
          {displayArray.length > 0 && (
            <div className="hidden md:flex absolute right-3 z-40 flex-col items-center gap-3" style={{ top: '50%', transform: 'translateY(-50%)' }}>
              ...
              <button
                ...
                className="relative flex flex-col items-center gap-1.5 p-2.5 rounded-full bg-[#1A1A1A]/80 backdrop-blur-md text-white hover:bg-[#222222] border border-white/10 transition-all shadow-lg hover:scale-110"
                aria-label="Shuffle"
                title="Shuffle"
              >
              ...
              <button
                ...
                className="relative flex flex-col items-center gap-1.5 p-2.5 rounded-full bg-[#A855F7] text-white hover:bg-[#9333EA] transition-all shadow-lg hover:scale-110"
                aria-label="Chat"
                title="Chat"
              >
              ...
              <button
                ...
                className={`relative flex flex-col items-center gap-1.5 p-2.5 rounded-full backdrop-blur-md transition-all shadow-lg hover:scale-110 ${
                  selectedTags.length > 0
                    ...
                    : 'bg-[#1A1A1A]/80 text-white hover:bg-[#222222] border border-white/10'
                }`}
                aria-label="Filter"
                title="Filter"
              >
      ...
      {displayArray.length > 0 && (
        <div className="fixed bottom-0 left-0 right-0 z-40 md:hidden bg-[#0A0A0A]/95 backdrop-blur-md border-t border-white/10">
          <div className="flex items-center justify-between py-4 px-4 gap-3">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-3 rounded-full bg-[#1A1A1A] border border-white/10 text-white font-semibold hover:bg-[#222222] transition-all shadow-lg"
              aria-label="Shuffle"
            >
              <Shuffle className="w-5 h-5" />
            ...
            <button
              ...
              className="flex items-center gap-2 px-6 py-3 rounded-full bg-[#A855F7] text-white font-semibold hover:bg-[#9333EA] transition-all shadow-lg flex-1 max-w-[200px] justify-center"
              aria-label="Chat"
            >
              <MessageCircle className="w-5 h-5" />
              <span>Chat</span>
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-3 rounded-full border border-white/10 text-white font-semibold transition-all shadow-lg ${
                selectedTags.length > 0
      ...
      {showFilterModal && (
        ...
        >
          ...
          {/* Modal Content */}
          <div
            className="relative w-full max-h-[80vh] bg-[#0A0A0A] rounded-t-3xl md:rounded-3xl border-t border-white/10 md:border md:border-white/10 md:max-w-md overflow-hidden"
          ...
          >
            ...
            <div className="flex items-center justify-between p-6 border-b border-white/10 bg-[#0A0A0A]">
              <h2 className="text-xl font-bold text-white tracking-tight">Filter by Tags</h2>
              <button
                onClick={() => setShowFilterModal(false)}
            ...
            {/* Filter Content */}
            <div className="overflow-y-auto max-h-[60vh] p-6 bg-[#141414]">
              {/* Popular Tags */}
              {popularTags.length > 0 && (
                <div className="mb-6">
                  <h3 className="text-sm font-semibold text-[#A1A1AA] mb-3">Popular</h3>
                        ...
                        <button
                          ...
                          className={`px-4 py-2 rounded-lg text-xs font-medium transition-all border ${
                            isSelected
                              ...
                              : 'bg-[#1A1A1A] text-[#A1A1AA] border-white/10 hover:bg-[#222222] hover:border-[#A855F7]/30'
                          }`}
                        >
              ...
              {/* All Tags */}
              <div>
                <h3 className="text-sm font-semibold text-[#A1A1AA] mb-3">All Tags</h3>
                <div className="flex flex-wrap gap-2">
                  {allTags.map((tag) => {
                    const isSelected = selectedTags.includes(tag.toLowerCase());
                    return (
              ...
              {selectedTags.length > 0 && (
                <div className="mt-6 pt-6 border-t border-white/10">
                  <button
                    ...
                    className="w-full px-4 py-3 rounded-xl bg-[#1A1A1A] text-white hover:bg-[#222222] border border-white/10 transition-colors font-medium"
                  >
                    Clear All Filters
                  </button>
                </div>
