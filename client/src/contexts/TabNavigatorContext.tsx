...
export type RootTabKey = "hangout" | "darkroom" | "campus" | "companion" | "profile";
...
export const TabNavigatorProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const initialRootTab: RootTabKey = useMemo(() => {
    ...
    if (location.pathname.startsWith("/arena/hangout")) return "hangout";
    if (location.pathname.startsWith("/arena")) return "companion";
    if (location.pathname.startsWith("/campus")) return "campus";
  ...
  const selectRootTab = useCallback(
    (tab: RootTabKey) => {
      ...
      switch (tab) {
        case "hangout":
          navigate("/arena/hangout", { replace: false });
          break;
        case "darkroom":
    ...
    (subTab: SubTabKey) => {
      ...
      switch (subTab) {
        ...
        case "my-chats":
          ...
          break;
        case "campus.info":
          navigate("/campus/info");
          break;
        case "campus.confessions":
          navigate("/campus/confessions");
          break;
      }
    },
    [navigate]
