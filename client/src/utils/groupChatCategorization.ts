export interface GroupChatCategory {
  id: string;
  name: string;
  icon: string;
  keywords: string[];
...
export const GROUP_CHAT_CATEGORIES: GroupChatCategory[] = [
  {
    id: 'anime-centric',
    name: 'Anime-centric',
    icon: '🌸',
...
export function categorizeGroupChat(name: string, description?: string, tags?: string[]): GroupChatCategory[] {
  const text = `${name} ${description || ''} ${tags?.join(' ') || ''}`.toLowerCase();
  const matchedCategories: GroupChatCategory[] = [];
